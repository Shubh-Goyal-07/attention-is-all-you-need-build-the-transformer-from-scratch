...
def decoder_layer_masked_self_attention_sublayer(y, w_q, w_k, w_v, w_o, gamma, beta, num_heads, tgt_mask):
    y_attn_ctx = assemble_multi_head_attention_forward(y, y, y, w_q, w_k, w_v, w_o, num_heads, tgt_mask)
    o = apply_residual_add_and_norm(y, y_attn_ctx, gamma, beta)
...
def decoder_layer_feed_forward_sublayer(y, w1, b1, w2, b2, gamma, beta):
    ...
    o = apply_residual_add_and_norm(y, y_ffn, gamma, beta)
    return o
...
def assemble_decoder_layer(y, encoder_output, layer_params, num_heads, src_mask, tgt_mask):
    ...
    y_attn_enc = decoder_layer_masked_self_attention_sublayer(
        y, 
        layer_params['w_q_self'], layer_params['w_k_self'], layer_params['w_v_self'], layer_params['w_o_self'], 
        layer_params['self_gamma'], layer_params['self_beta'],
        num_heads, tgt_mask
    )

    y_attn_enc_2 = decoder_layer_cross_attention_sublayer(
        y_attn_enc, encoder_output, 
        layer_params['w_q_cross'], layer_params['w_k_cross'], layer_params['w_v_cross'], layer_params['w_o_cross'], 
        layer_params['cross_gamma'], layer_params['cross_beta'],
        num_heads, src_mask
    )

    o = decoder_layer_feed_forward_sublayer(
        y_attn_enc_2, 
        layer_params['w1'], layer_params['b1'], 
        layer_params['w2'], layer_params['b2'], 
        layer_params['ffn_gamma'], layer_params['ffn_beta'])
...
def stack_decoder_layers(y, encoder_output, decoder_layer_params_list, num_heads, src_mask, tgt_mask):
    for i, layer_params in enumerate(decoder_layer_params_list):
...
def run_transformer_forward(src_ids, tgt_ids, model_params, num_heads, pad_id):
    token_embedd_weights = model_params["token_embedding"]
    vocab_size, d_model = token_embedd_weights.shape

    B, src_seq_len = src_ids.shape
    _, tgt_seq_len = tgt_ids.shape

    # Make Embedding from ids
    src_embed = token_embedd_weights[src_ids]
    tgt_embed = token_embedd_weights[tgt_ids]
    # Scale Embeddings
    src_embed = scale_embeddings_by_sqrt_d_model(src_embed, d_model)
    tgt_embed = scale_embeddings_by_sqrt_d_model(tgt_embed, d_model)
    
    # Compute Positional Encoding
    pe = build_sinusoidal_positional_encoding(max(src_seq_len, tgt_seq_len), d_model)
    # Add positional encodings
    src_embed = add_positional_encoding_to_embeddings(src_embed, pe)
    tgt_embed = add_positional_encoding_to_embeddings(tgt_embed, pe)

    # Build Masks
    src_mask = build_padding_mask(src_ids, pad_id)
    tgt_mask = combine_padding_and_causal_masks(
        build_padding_mask(tgt_ids, pad_id),
        build_causal_mask(tgt_seq_len)
    )

    # Run Encoder
    x_enc = stack_encoder_layers(src_embed, model_params["encoder_layers"], num_heads, src_mask)
    # Run Decoder
    y_enc = stack_decoder_layers(tgt_embed, x_enc, model_params["decoder_layers"], num_heads, src_mask, tgt_mask)

    # Run output FFN
    logits = apply_final_output_projection(y_enc, model_params["output_projection"])
    outs = apply_log_softmax_over_vocab(logits)
...
def init_embedding_and_projection_parameters(vocab_size, d_model, tie_weights=True):
    ...
    src_embedding = xavier_tensor((vocab_size, d_model))
    tgt_embedding = xavier_tensor((vocab_size, d_model))
    output_projection = tgt_embedding if tie_weights else xavier_tensor((vocab_size, d_model))
...
def shift_targets_right_with_start_token(target_ids, start_token_id):
    target_ids_shifted = torch.empty_like(target_ids)
    target_ids_shifted[:, 0] = start_token_id
    target_ids_shifted[:, 1:] = target_ids[:, :-1]
    return target_ids_shifted

# Step 57 - compute_noam_learning_rate
def compute_noam_learning_rate(step, d_model, warmup_steps):
    step_factor = min(step**(-1/2), step*(warmup_steps**(-3/2)))
...
def set_confidence_on_gold_tokens(smoothed_distribution, gold_token_ids, confidence):
    ...
    output.scatter_(2, gold_token_ids.unsqueeze(-1), confidence)
    return output
...
def average_loss_over_non_pad_tokens(total_loss, gold_token_ids, pad_id):
    ...
    avg_loss = total_loss / (denom if denom else 1)
    return avg_loss
...
def compute_token_accuracy_ignoring_pad(log_probabilities, gold_token_ids, pad_id):
    pred_ids = torch.argmax(log_probabilities, dim=-1)
    pad_mask = ~(gold_token_ids==pad_id)
    acc_mat = (pred_ids == gold_token_ids) * pad_mask
    total = torch.sum(pad_mask)
    correct = torch.sum(acc_mat)
    return torch.tensor(correct / (total if total else 1))
...
def initialize_adam_optimizer_state(parameter_list):
    ...
    optim_dict = {
        "m": [torch.zeros_like(param, dtype=torch.float32) for param in parameter_list],
        "v": [torch.zeros_like(param, dtype=torch.float32) for param in parameter_list],
        "t": 0
    }
...
def apply_adam_step_to_all_parameters(parameter_list, optimizer_state, learning_rate, beta1=0.9, beta2=0.98, epsilon=1e-9):
    ...
    for i, param in enumerate(parameter_list):
        ...
        if param_grad is None:
            continue
        m_t = update_adam_first_moment(optimizer_state["m"][i], param_grad, beta1)
        v_t = update_adam_second_moment(optimizer_state["m"][i], param_grad, beta2)

        m_hat, v_hat = apply_adam_bias_correction(m_t, v_t, beta1, beta2, t)

        optimizer_state["m"][i] = m_hat
        optimizer_state["v"][i] = v_hat
